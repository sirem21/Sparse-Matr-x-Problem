...
#find the estimated time and relative residuals for each K part for Kaczmarz method
def time_residual_kaczmarz(A, f, K):
    start_time = time.time()
    x = kaczmarz(A, f, K, 1000)
    end_time = time.time()
    elapsed_time = end_time - start_time
    residual = norm(A.dot(x) - f) / norm(f)
    return elapsed_time, residual

#find residual and estimated time for each K-parts
for K in [2, 4, 6, 8]:
    t,res=time_residual_cimmino(A, f, K)
    print(f"Cimmino Method with K={K}  Residual: {res} Time:{t}")
for K in [2, 4, 6, 8]:
    t, res = time_residual_kaczmarz(A, f, K)
    residual2 = norm(A.dot(kaczmarz(A, f, K,1000)) - f) / norm(f)
    print(f"Kaczmarz Method with K={K} Residual: {res}  Time:{t}")

#solving the system by using the scipy least square method
#find the elapsed time and relative residual for least square method
